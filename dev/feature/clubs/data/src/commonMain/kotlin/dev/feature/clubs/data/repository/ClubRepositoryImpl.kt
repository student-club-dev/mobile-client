package dev.feature.clubs.data.repository
...
class ClubRepositoryImpl(
    ...
    override fun observeClubs(): Flow<List<Club>> =
        database.clubQueries.selectAll().asFlow().mapToList(dispatchers.io)
    ...
    override suspend fun refreshClubs(): Resource<List<Club>> = try {
        ...
        queries.transaction {
            queries.deleteAll()
            dtos.forEach { queries.upsert(it.id, it.name, it.description, it.membersCount.toLong(), it.imageUrl) }
        }
